...
-->
        <div>ID : </div><div id="companyUserID"></div>&nbsp;&nbsp;&nbsp;
        <div>ContactName : </div><div id="companyUserContactName"></div>
    </div>
  </div>
</template>
...
<script>
    ...
    var useraccount = nav_module.data.jsonparse(window.Laravel.useraccount);
    ...
    export default {
        ...
        methods:{
            initpage(){
                $('#companyUserID').text(atob(useraccount.uid));
                $('#companyUserContactName').text(atob(useraccount.uname));
            },
        }
    };
</script>
